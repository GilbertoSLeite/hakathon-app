...
<script>
import { findSectorialGoals } from 'src/services/goals-service/crud-goals-service';
import { ref } from 'vue';
...
const columns = [
  ...
  { name: 'maximo', label: 'Máximo', field: 'maximo' },
];
...
export default {
  ...
  methods: {
    async listSectorialGoals() {
      this.isLoading = true;

      const token = localStorage.getItem('token');
      const decoded = jwt_decode(token);
      const goals = await findSectorialGoals(2, decoded.setorId);
